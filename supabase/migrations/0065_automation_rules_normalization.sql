-- supabase/migrations/0065_automation_rules_normalization.sql
--
-- CommunicationAutomationRule: { id, name, trigger, templateName,
--   channels: string[], enabled }
-- channels stored as JSONB array. Idempotent.
...
create or replace function public.automation_rules_set_updated_at()
...
for each row execute function public.automation_rules_set_updated_at();

-- Load RPC
create or replace function app.load_automation_rules()
...
$$;
grant execute on function app.load_automation_rules() to authenticated;

-- Upsert single
create or replace function app.upsert_automation_rule(p_a jsonb)
returns jsonb language plpgsql security definer
set search_path = app, public
as $$
declare v_id text := p_a->>'id';
begin
  if v_id is null then raise exception 'automation_rule.id is required'; end if;
  insert into public.automation_rules (id, name, trigger, template_name, channels, enabled, updated_at)
  values (
    v_id,
    coalesce(p_a->>'name', ''),
    coalesce(p_a->>'trigger', ''),
    coalesce(p_a->>'templateName', ''),
    coalesce(p_a->'channels', '[]'::jsonb),
    coalesce((p_a->>'enabled')::boolean, true),
    now()
  )
...
$$;
grant execute on function app.upsert_automation_rule(jsonb) to authenticated, service_role;

-- Bulk upsert
create or replace function app.upsert_automation_rules_bulk(p_rules jsonb)
...
begin
  ...
  loop perform app.upsert_automation_rule(v_item); v_count := v_count + 1; end loop;
  return v_count;
end;
$$;
grant execute on function app.upsert_automation_rules_bulk(jsonb) to authenticated, service_role;
...
begin
  for r in select value from app.app_state where key = 'tm_automation_rules_v11'
  loop
    v_arr := r.value;
    ...
    loop
      ...
      exception when others then
        raise notice 'Skipped automation_rule id=% reason=%', v_item->>'id', sqlerrm;
        v_sk := v_sk + 1;
      end;
    end loop;
  end loop;
  raise notice 'Automation rules backfill complete: upserted=%, skipped=%', v_up, v_sk;
end $$;
...
as $$
  select coalesce(
    jsonb_object_agg(key, value) filter (where key not in (
      ...
      'tm_workflows_v11','tm_automation_rules_v11'
    )), '{}'::jsonb
  )
  || jsonb_build_object(
