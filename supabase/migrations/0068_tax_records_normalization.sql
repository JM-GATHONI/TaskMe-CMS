-- supabase/migrations/0068_tax_records_normalization.sql
--
-- TaxRecord: { id, type, description, amount, date, status }
-- Idempotent.
...
create index if not exists tax_records_date_idx   on public.tax_records (date desc);
create index if not exists tax_records_status_idx on public.tax_records (status);

alter table public.tax_records enable row level security;

drop policy if exists tax_records_select_admin on public.tax_records;
create policy tax_records_select_admin
on public.tax_records for select to authenticated
using (app.is_admin(auth.uid()));

drop policy if exists tax_records_write_admin on public.tax_records;
create policy tax_records_write_admin
on public.tax_records for all to authenticated
using (app.is_admin(auth.uid()))
with check (app.is_admin(auth.uid()));

create or replace function public.tax_records_set_updated_at()
...
for each row execute function public.tax_records_set_updated_at();

-- Load RPC
create or replace function app.load_tax_records()
...
$$;
grant execute on function app.load_tax_records() to authenticated;

-- Upsert single
create or replace function app.upsert_tax_record(p_t jsonb)
returns jsonb language plpgsql security definer
set search_path = app, public
as $$
declare v_id text := p_t->>'id';
begin
  if v_id is null then raise exception 'tax_record.id is required'; end if;
  insert into public.tax_records (id, type, description, amount, date, status, updated_at)
  values (
    v_id,
    coalesce(p_t->>'type', ''),
    coalesce(p_t->>'description', ''),
    coalesce(nullif(p_t->>'amount', '')::numeric, 0),
    coalesce(p_t->>'date', ''),
    coalesce(p_t->>'status', 'Due'),
    now()
  )
...
$$;
grant execute on function app.upsert_tax_record(jsonb) to authenticated, service_role;

-- Bulk upsert
create or replace function app.upsert_tax_records_bulk(p_records jsonb)
...
begin
  ...
  loop perform app.upsert_tax_record(v_item); v_count := v_count + 1; end loop;
  return v_count;
end;
$$;
grant execute on function app.upsert_tax_records_bulk(jsonb) to authenticated, service_role;
...
begin
  for r in select value from app.app_state where key = 'tm_tax_records_v11'
  loop
    v_arr := r.value;
    ...
    loop
      ...
      exception when others then
        raise notice 'Skipped tax_record id=% reason=%', v_item->>'id', sqlerrm;
        v_sk := v_sk + 1;
      end;
    end loop;
  end loop;
  raise notice 'Tax records backfill complete: upserted=%, skipped=%', v_up, v_sk;
end $$;
...
as $$
  select coalesce(
    jsonb_object_agg(key, value) filter (where key not in (
      ...
      'tm_scheduled_reports_v11','tm_tax_records_v11'
    )), '{}'::jsonb
  )
  || jsonb_build_object(
