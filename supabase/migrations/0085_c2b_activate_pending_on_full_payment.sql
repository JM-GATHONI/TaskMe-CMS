-- Migration 0085: activate Pending/PendingPayment tenants in record_c2b_payment
--
-- Problem: record_c2b_payment (migration 0077) runs the full priority waterfall
-- (deposit → late fees → arrears → rent → bills) but never transitions a
-- Pending/PendingPayment tenant to Active, even when the payment covers both
-- rent and the full deposit. The tenant stays stuck in PendingPayment forever
-- until someone manually reconciles via the UI.
--
-- Fix: after the waterfall completes, re-read the tenant row and apply the
-- same activation logic used in match_c2b_payment_to_tenant and
-- record_manual_payment (migration 0084):
--   if status in ('Pending','PendingPayment') and payment >= rent + deposit
--   → set status = 'Active', activation_date = payment date.
...
begin
  ...
  if v_ft_tenant_id is not null then
    ...
    -- ── 5. Status activation (NEW — migration 0085) ──────────────────────────
    -- Re-read tenant after waterfall updates to get fresh deposit_paid etc.
    select * into v_tenant_row from public.tenants where id = v_ft_tenant_id;

    if v_tenant_row.status in ('Pending', 'PendingPayment') then
      -- Compute expected deposit (same logic as match_c2b_payment_to_tenant)
      v_dep_expected := case
        when coalesce(v_tenant_row.deposit_exempt, false)                                   then 0
        when coalesce(v_tenant_row.rent_extension->>'enabled', 'false') = 'true'            then 0
        when coalesce(v_tenant_row.prorated_deposit->>'enabled', 'false') = 'true'
             then coalesce((v_tenant_row.prorated_deposit->>'monthlyInstallment')::numeric, 0)
        when coalesce(v_tenant_row.deposit_expected, 0) > 0                                 then v_tenant_row.deposit_expected
        else coalesce(v_tenant_row.rent_amount, 0) * coalesce(v_tenant_row.deposit_months, 1)
      end;

      -- Activate when payment covers rent + full deposit
      if p_amount >= coalesce(v_tenant_row.rent_amount, 0) + v_dep_expected then
        update public.tenants
           set status          = 'Active',
               activation_date = to_char(v_created_at, 'YYYY-MM-DD'),
               updated_at      = now()
         where id = v_ft_tenant_id;
      end if;
    end if;
