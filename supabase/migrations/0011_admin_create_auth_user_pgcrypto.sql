-- Fix: "function gen_salt(unknown) does not exist" on manual user registration (production).
-- Ensures pgcrypto is present and hashes passwords with schema-qualified crypt/gen_salt.
...
begin
  ...
  v_encrypted_pw := extensions.crypt(p_password, extensions.gen_salt('bf'::text));
