...
as $$
declare
  -- Scalars for the existing-row lookup (avoids record-field references
  -- inside SQL subqueries, which PostgreSQL can misparse as table names).
  v_existing_payment_id  uuid;
  v_existing_tenant_id   text;
  v_existing_unit_id     text;
  -- Scalars for the unit-tag tenant match.
  v_match_tenant_id      text;
  v_match_unit_id        text;
  -- Working vars.
  v_payment_id           uuid;
  v_status               text;
  ...
  -- Race-recovery vars (conflict re-select).
  v_race_payment_id      uuid;
  v_race_tenant_id       text;
  v_race_unit_id         text;
begin

  -- ── Idempotency check ────────────────────────────────────────────────────
  select id, matched_tenant_id, matched_unit_id
    into v_existing_payment_id, v_existing_tenant_id, v_existing_unit_id
  ...
  if v_existing_payment_id is not null then
    ...
    -- Resolve the tenant via bill ref (in case the STK row had no match yet).
    select tenant_id, unit_id
      into v_match_tenant_id, v_match_unit_id
    from public.find_active_tenant_by_unit_tag(p_bill_ref)
    limit 1;

    v_resolved_tenant_id := coalesce(v_existing_tenant_id, v_match_tenant_id);
    v_resolved_unit_id   := coalesce(v_existing_unit_id,   v_match_unit_id);

    -- Mirror C2B metadata onto the existing (STK) row.
    update public.payments
       set bill_ref_number     = coalesce(bill_ref_number,     p_bill_ref),
           ...
           matched_tenant_id   = coalesce(matched_tenant_id,   v_match_tenant_id),
           matched_unit_id     = coalesce(matched_unit_id,     v_match_unit_id),
    ...
    -- Use jsonb @> containment in the WHERE clause instead of a
    -- NOT EXISTS / jsonb_array_elements subquery — PostgreSQL can
    -- misinterpret PL/pgSQL variable names as table references inside
    -- those nested FROM clauses.
  ...
  v_status := case
    when v_match_tenant_id is not null then 'reconciled'
    else 'unreconciled'
  end;
  ...
  insert into public.payments (
    ...
    p_org_balance, v_match_tenant_id, v_match_unit_id,
  ...
  if v_payment_id is null then
    -- Lost a race — re-select the winner row.
    select id, matched_tenant_id, matched_unit_id
      into v_race_payment_id, v_race_tenant_id, v_race_unit_id
    from public.payments
    where transaction_id = p_transaction_id
    limit 1;

    payment_id        := v_race_payment_id;
    was_duplicate     := true;
    matched_tenant_id := v_race_tenant_id;
    matched_unit_id   := v_race_unit_id;
    return next;
    return;
  end if;

  -- ── Pair with a recent STK row if one exists ─────────────────────────────
  if v_match_tenant_id is not null then

    select id into v_twin_id
    from public.payments
    where source = 'stk'
      and matched_tenant_id is not distinct from v_match_tenant_id
    ...
    -- Fallback: match by msisdn when the STK row had no tenant yet.
    ...
    update public.tenants
       set payment_history = jsonb_build_array(
           ...
           updated_at = now()
     where id = v_match_tenant_id;

  end if;

  payment_id        := v_payment_id;
  was_duplicate     := false;
  matched_tenant_id := v_match_tenant_id;
  matched_unit_id   := v_match_unit_id;
  return next;
end;
$$;
